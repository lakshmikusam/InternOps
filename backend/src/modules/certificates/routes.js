...
async function routes(fastify) {
  ...
  fastify.get('/templates', {
    schema: { tags: ['Certificates'], description: 'List certificate templates' },
  }, async (req) => {
  ...
  fastify.post('/templates', {
    schema: { tags: ['Certificates'], description: 'Create certificate template' },
  }, async (req, reply) => {
  ...
  fastify.put('/templates/:id', {
    schema: { tags: ['Certificates'], description: 'Update certificate template' },
  }, async (req, reply) => {
  ...
  fastify.delete('/templates/:id', {
    schema: { tags: ['Certificates'], description: 'Delete certificate template' },
  }, async (req, reply) => {
  ...
  fastify.post('/generate', {
    schema: { tags: ['Certificates'], description: 'Generate a single certificate' },
  }, async (req, reply) => {
    ...
    req.auditOnResponse = {
      ...
      details: { recipient: data.recipient_name, type: data.certificate_type },
      ipAddress: req.ip,
  ...
  fastify.delete('/:id', {
    schema: { tags: ['Certificates'], description: 'Delete/revoke certificate' },
  }, async (req, reply) => {
  ...
  fastify.post('/bulk/generate', {
    schema: { tags: ['Certificates'], description: 'Bulk generate certificates' },
  }, async (req, reply) => {
    ...
    req.auditOnResponse = {
      ...
      details: { total: result.data.total, generated: result.data.generated, failed: result.data.failed },
      ipAddress: req.ip,
      userAgent: req.headers['user-agent'],
    };
  ...
  fastify.post('/ai/generate-content', {
    schema: { tags: ['Certificates', 'AI'], description: 'AI-generate certificate content' },
  }, async (req) => {
    const data = aiGenerateContentSchema.parse(req.body);
    return service.generateAIContent(data);
  });

  fastify.post('/ai/suggest-template', {
    schema: { tags: ['Certificates', 'AI'], description: 'AI-suggest best template' },
  }, async (req) => {
  ...
  fastify.post('/quick-generate', {
    schema: { tags: ['Certificates'], description: 'Quick generate certificate with name, domain, dates' },
  }, async (req, reply) => {
    const { template_id, recipient_name, domain, start_date, end_date, issuer } = req.body;

    if (!recipient_name || !domain || !start_date || !end_date) {
      return reply.code(400).send({ error: 'recipient_name, domain, start_date, and end_date are required' });
    }
    ...
    req.auditOnResponse = {
      ...
      resourceType: 'certificate',
      resourceId: result.data.id,
      details: { recipient: recipient_name, domain, cert_number: result.data.certificate_number },
      ipAddress: req.ip,
      userAgent: req.headers['user-agent'],
    };
